...
flags.DEFINE_string('output', './data/video/result_out.avi',
...
def main(_argv):
    ...
    while True:
        ...
        for i in banma:
            i = eval(i)
            cv2.rectangle(img, (i['x1'], i['y1']+i['c']),
                          (i['x2'], i['y2']), (0, 255, 0), 4)  # 画斑马线
        for i in k:
            i = eval(i)
            if i['k'] != 0:
                for xxx in range(i['y1'], i['y2']):
                    xq = (xxx-i['b'])/i['k']
                    xq = int(xq)
                    cv2.rectangle(img, (xq+5, xxx),
                                  (xq-5, xxx), (0, 0, 255), -1)
            else:
                for xxx in range(i['x1'], i['x2']):
                    yq = i['b']
                    cv2.rectangle(img, (xxx, yq+5),
                                  (xxx, yq-5), (0, 0, 255), -1)
                ...
                for ix in banma:
                    ...
                    if on_banmaxian(ix['k'], ix['b'], ix['c'], ix['x1'], ix['x2'], ix['y1'], ix['y2'], x, y):
                        if len(on_bmx) != 0:
                            ...
                            if str(track.track_id) in chepaixinxi:
                                img = cv2ImgAddText(img, '不礼让行人', int(
                                    bbox[0]), int(bbox[1]+40), (255, 0, 0), 20)
                                print('car违规:没有礼让行人!',
                                      chepaixinxi[str(track.track_id)][0])
                            else:
                                img = cv2ImgAddText(img, '不礼让行人', int(
                                    bbox[0]), int(bbox[1]+40), (255, 0, 0), 20)
                                print('car违规:没有礼让行人!')
